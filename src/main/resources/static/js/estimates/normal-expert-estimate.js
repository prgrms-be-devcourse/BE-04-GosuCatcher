...
$(document).ready(function () {
    $("#requestButton").click(function () {
        ...
        var memberEstimateId = $("#memberEstimateId").val();
        ...
        $.ajax({
            type: "POST",
            url: "/api/v1/expert-estimates/normal?memberEstimateId=" + memberEstimateId,
            headers: headers,
            contentType: "application/json",
            data: JSON.stringify(expertEstimateRequest),
            success: function (response) {
                window.alert("일반 견적이 성공적으로 생성되었습니다.");
                window.location.href = '/gosu-catcher';
            },
            error: function (xhr, status, error) {
                window.alert("일반 견적 생성에 실패했습니다.");
            }
        });
    });
});
...
window.onload = () => {
    checkToken();
    var memberEstimateId = document.getElementById("memberEstimateId").value;
    loadMemberEstimate(memberEstimateId);
};
...
async function loadMemberEstimate(memberEstimateId) {
    try {
        ...
        const response = await fetch("/api/v1/member-estimates/" + memberEstimateId, options);
        document.getElementById('memberEstimateId').innerText = memberEstimateId;
        const memberEstimate = await response.json();
        displayMemberEstimate(memberEstimate);
    } catch (error) {
        console.error('회원 요청 견적서 로딩 중 오류 발생:', error);
    }
}
